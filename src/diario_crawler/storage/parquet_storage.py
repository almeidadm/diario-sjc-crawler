...
class ParquetStorage(BaseStorage):
        ...
        for edition in editions:
            ...
            for article in edition.articles:
                ...
                try:
                    ...
                    logger.warning(
                        f"Erro ao decodificar conteúdo do artigo {article.article_id}: {e}"
                    )
                    raw_content = ""
                ...
                articles_data.append(
                    {
                        ...
                        "hierarchy_path": (
                            json.dumps(article.hierarchy_path)
                            if article.hierarchy_path
                            else "[]"
                        ),
                        "identifier": (
                            str(article.metadata.identifier)
                            if article.metadata.identifier
                            else ""
                        ),
                        "protocol": (
                            str(article.metadata.protocol)
                            if article.metadata.protocol
                            else ""
                        ),
    ...
    def _generate_edition_hash(self, edition: GazetteEdition) -> str:
        ...
        content = (
            f"{edition.edition_id}_{edition.publication_date}_{len(edition.articles)}"
        )
    ...
    def save_editions(self, editions: list[GazetteEdition], **kwargs: Any) -> None:
        ...
        try:
            ...
            if not editions_df.empty:
                editions_df["batch_id"] = batch_id
                editions_path = (
                    self.base_path / "gazettes" / f"editions_{timestamp}.parquet"
                )
    ...
    def _save_edition_article_relationship(
        self, editions: list[GazetteEdition], batch_id: str, timestamp: str
    ) -> None:
        ...
        if relationship_data:
            ...
            relationship_path = (
                self.base_path
                / "relationships"
                / f"edition_articles_{timestamp}.parquet"
            )
    ...
    def _load_articles_for_edition(self, edition_hash: str) -> list[Article]:
        ...
        try:
            ...
            edition_articles_df = articles_df[
                articles_df["edition_hash"] == edition_hash
            ]
            ...
            for _, article_row in edition_articles_df.iterrows():
                try:
                    ...
                    article_metadata = ArticleMetadata(
                        ...
                        identifier=(
                            str(article_row["identifier"])
                            if article_row["identifier"]
                            else None
                        ),
                        protocol=(
                            str(article_row["protocol"])
                            if article_row["protocol"]
                            else None
                        ),
                    )
                    ...
                    logger.error(
                        f"Erro ao carregar artigo {article_row['article_id']}: {e}"
                    )
                    continue

        except Exception as e:
            logger.error(
                f"Erro ao carregar artigos para edition_hash {edition_hash}: {e}"
            )
